...
namespace ETC1Converter
{
    public class TextureEncoderDecoder
    {
        ...
        {
            var dataIntPtr = Marshal.AllocHGlobal(data.Length);
            Marshal.Copy(data, 0, dataIntPtr, data.Length);
            var destIntPtr = Marshal.AllocHGlobal(width * height * 4);            
            
            var size = PInvoke.DecodeByPVRTexLib(dataIntPtr, destIntPtr, (int)format, (uint)width, (uint)height);
            Marshal.FreeHGlobal(dataIntPtr);

            if (size > 0)
            {
                byte[] resizedDest = new byte[size];
                Marshal.Copy(destIntPtr, resizedDest, 0, resizedDest.Length);
                Marshal.FreeHGlobal(destIntPtr);
                return resizedDest;
            }
            else
            {
        ...
        private static byte[] DecodeCrunch(byte[] data, int width, int height, TextureFormat format)
        {
            var dataIntPtr = Marshal.AllocHGlobal(data.Length);
            Marshal.Copy(data, 0, dataIntPtr, data.Length);
            var oldSize = width * height * 4;
            var destIntPtr = Marshal.AllocHGlobal(oldSize);

            var size = PInvoke.DecodeByCrunchUnity(dataIntPtr, destIntPtr, (int)format, (uint)width, (uint)height, (uint)data.Length);
            Marshal.FreeHGlobal(dataIntPtr);

            if (size > 0)
            {
                byte[] dest = new byte[oldSize];
                Marshal.Copy(destIntPtr, dest, 0, dest.Length);
                Marshal.FreeHGlobal(destIntPtr);
                return dest;
            }
            else
        ...
        private static byte[] EncodeISPC(byte[] data, int width, int height, TextureFormat format, int quality)
        {
            ...
            var size = PInvoke.EncodeByISPC(dataIntPtr, destIntPtr, (int)format, quality, (uint)width, (uint)height);
            Marshal.FreeHGlobal(dataIntPtr);
        ...
        private static byte[] EncodePVRTexLib(byte[] data, int width, int height, TextureFormat format, int quality)
        {
            var dataIntPtr = Marshal.AllocHGlobal(data.Length);
            Marshal.Copy(data, 0, dataIntPtr, data.Length);
            var destIntPtr = Marshal.AllocHGlobal(width * height * 16); //just to be safe, buf is 16 times size of original (obv this is prob too big)
            
            var size = PInvoke.EncodeByPVRTexLib(dataIntPtr, destIntPtr, (int)format, quality, (uint)width, (uint)height);
            Marshal.FreeHGlobal(dataIntPtr);
        ...
        private static byte[] EncodeCrunch(byte[] data, int width, int height, TextureFormat format, int quality)
        {
            ...
            var size = PInvoke.EncodeByCrunchUnity(dataIntPtr, destIntPtr, (int)format, quality, (uint)width, (uint)height, 1);
            Marshal.FreeHGlobal(dataIntPtr);
